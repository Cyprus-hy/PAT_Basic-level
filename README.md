...
1068 简单题 这道题很坑，因为题目要求的“一点红”明明是该点的颜色与其周围8个相邻像素的颜色差充分大，但实际上测试样例里是有边缘像素点的。对于边缘像素点来说，默认它和它的那些不存在的方向上的像素点色差充分大。在开数组的时候，我们可以开大两个长度，并初始化所有像素值充分小，这样就不用判断边界情况了。还有一点就是记录每个颜色出现的次数可以用map
